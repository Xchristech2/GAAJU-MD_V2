...
async function pairCommand(sock, chatId, message, q) {
    // Shared context info with GAAJU-MD branding
    const newsletterMeta = {
        forwardingScore: 1,
        isForwarded: true,
        forwardedNewsletterMessageInfo: {
            newsletterJid: '@newsletter',
            newsletterName: '🛠️ GAAJU-BOT',
            serverMessageId: -1
        }
    };
    ...
    try {
        ...
        if (numbers.length === 0) {
            return await sock.sendMessage(chatId, {
                text: "❌ Invalid number format. Please use something like `234801234567`.",
                contextInfo: newsletterMeta
            });
        }
