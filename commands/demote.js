    ...
    try {
        ...
        try {
            ...
            if (!adminStatus.isBotAdmin) {
                await sock.sendMessage(chatId, {
                    text: '🔒 *GAAJU MD* needs admin powers to swing the demotion hammer! 🔨'
                });
                return;
            }
            ...
            await sock.sendMessage(chatId, {
                text: '⚠️ GAAJU MD can’t check admin status. Make sure the bot has permissions.'
            });
            return;
        }
        ...
        if (error.data === 429) {
            ...
            try {
                await sock.sendMessage(chatId, {
                    text: '❌ GAAJU MD failed to demote! Is the user already powerless or bot not admin?'
                });
            } catch (sendError) {
...
async function handleDemotionEvent(sock, groupId, participants, author) {
    try {
        ...
        const demotionMessage = `💢 *AUTO-DEMOTION DETECTED BY GAAJU MD* 💢\n\n` +
